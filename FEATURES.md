...
| 引擎 | 触发条件 | 能力 |
|---|---|---|
| E2 增强 redefine（主线） | DCEVM(-XXaltjvm=dcevm) / JBR 17+(-XX:+AllowEnhancedClassRedefinition)，自动注入 | 增强运行时接受的结构变更保持类身份，实例与状态保留 |
| E1 标准 redefine | 所有 JVM | 方法体/常量修改 |
| E3 generation 降级 | 无增强运行时时的结构变更 | 仅支持可表达的新增成员，且限成功重建直接 Spring Bean |

## 能力矩阵

### A. Java 类
- ✅ 方法体修改（E1/E2，全部实例即时生效）
- ✅ 新增/删除方法、修改签名（增强运行时接受时由 E2 就地生效）
- ✅ 新增/删除字段、修改字段类型（增强运行时接受时由 E2 就地生效；已存实例新字段为默认值）
- ◐ E3 仅降级处理可表达的新增方法/字段，并要求对应直接 Spring Bean 成功重建
- ◐ 删除或改签名方法、删除或改类型字段、父类变化、既有接口移除（E3 明确要求重启）
- ◐ 新增类可在真实应用类加载器中安全定义；无条件 @Component 可自动注册，带 @Profile/@Conditional 的组件需重启
...
- ◐ Mapper XML 文件或目录新增、删除、移动、重命名，以及内容类型切换 → 提示需重启
